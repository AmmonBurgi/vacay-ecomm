module.exports={
    addToCart: async(req, res) => {
        ...
        if(existingProduct[0]){
            ...
            return (
                db.cart.updateQuantity(newQuantity, product_id, user_id).then(() => {
                    res.status(200).send('Product added to cart!')
                }).catch(err => res.status(500).send(err))
            )
        }

        db.cart.add_cart(user_id, product_id, addQuantity, product_title, product_price, product_img, type_id ).then(() => {
            res.status(200).send('Product added to cart!')
        }).catch(err => res.status(500).send(err))
    }
}